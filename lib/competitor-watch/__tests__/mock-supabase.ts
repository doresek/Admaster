// Minimal in-memory Supabase stub for the competitor-watch tests.
//
// Implements exactly the PostgREST chain lib/competitor-watch and the
// lib/intelligence data layer use — select / insert (single row AND batch
// array) / update, eq / order / limit, single / maybeSingle / overrideTypes,
// thenable execution — over plain in-memory tables, logging every write so
// tests can assert idempotency by op count. Adapted from
// lib/voc/__tests__/mock-supabase.ts — kept local per the capability-folder
// collision doctrine.
...
interface HarnessState {
  ...
  log:    string[];
  failOn: Set<string>; // e.g. 'insert:competitor_ads', 'select:client_insights'
  seq:    number;
}
...
export function mockSupabase(): SupabaseMock {
  ...
  // the repo's ONE tolerated documented cast (mirrors lib/voc's and
  // lib/hypotheses' mock-supabase.ts).
