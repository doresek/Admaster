...
## CONTENT PIPELINE & LEARNING — HIGH-PRIORITY cluster, next after the current build (owner-directed 2026-07-06)

> **One flow:** create (batch, background, PERSISTED) → approve (see the full ad, feel it) → publish (from approved) → learn (every choice, approval, edit, and result makes the next generation smarter). Takes precedence over remaining P1/P2/P3 once the current cluster lands.
