...
**NEXT (in order):** (1) P1-5 calendar UI (`app/(dashboard)/calendar/`) — plan/generate/review per autonomy mode. (2) CP-6b T5 series-page wiring (`app/(dashboard)/series/`) + T6 opt-out endpoint/page. (3) P1-7 organic perf (`lib/organic-perf/`). (4) U-1 sidebar organic section (orchestrator-serial). **Waiting on owner:** G0-1 Business Verification submission (workbook ready); Nano-Banana billing (~the 10th); CP-4b routed around (LP=planning's). Money gate intact; self-campaign PAUSED; no destructive migrations pending.
