...
5. **Autonomy model — DECIDED (Eliran, 2026-07-04, VISION-DEEP D1):** 3 user-selectable MODES per client replace the L0–L3 ladder — `draft_only` · `propose_approve` (default) · `act_within_caps` (user-set caps). `lib/autonomy` is the gate every publish/spend action routes through (the heartbeat calls it for everything); graduation logic emits mode-switch *suggestions* only, never automatic promotion.
